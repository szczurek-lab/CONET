...
namespace Gauss {
	...
	template <class Real_t> void truncated_gaussian_log_likelihood(std::vector<Real_t> &result, const std::vector<Real_t> &args, const Real_t mean, const Real_t sd) {
	...
	template <class Real_t> Real_t truncated_gaussian_log_likelihood(const Real_t arg, const Real_t mean, const Real_t sd) {
		const Real_t log_inv_sqrt_2pi = -0.9189385;
