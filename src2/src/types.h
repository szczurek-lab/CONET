...
inline Locus get_event_end_locus(Event event) {
	return event.second;
}
...
inline TreeLabel get_root_label() {
	return std::make_pair(0, 0);
}

inline std::string label_to_str(TreeLabel label) {
	return "(" + std::to_string(label.first) + "," + std::to_string(label.second) + ")";
}

inline TreeLabel label_from_str(std::string l) {
	l.erase(std::remove(l.begin(), l.end(), '('), l.end());
	l.erase(std::remove(l.begin(), l.end(), ')'), l.end());
	return std::make_pair( (size_t) std::stoull(l.substr(0, l.find(','))), (size_t) std::stoull(l.substr(l.find(',') + 1, l.length())));
}
