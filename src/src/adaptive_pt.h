...
template <class Real_t> class AdaptivePT {
  ...
  std::vector<Real_t> get_H_vector(const std::vector<Real_t> &states) const {
    ...
    for (size_t i = 0; i < p_constants.size(); i++) {
      result.push_back((temperatures[i] - temperatures[i + 1]) *
                       (states[i + 1] - states[i]));
    }
    for (auto &el : result) {
      if (el >= 0.0) {
...
public:
  ...
  std::vector<Real_t> get_temperatures() const {
    ...
    for (size_t i = 0; i < p_constants.size(); i++) {
      temperatures.push_back(temperatures.back() *
                             std::exp(-std::exp(p_constants[i])));
    }
    return temperatures;
  }
  ...
  void update(const std::vector<Real_t> &states) {
    ...
    for (size_t i = 0; i < p_constants.size(); i++) {
      p_constants[i] =
          p_constants[i] +
          step_size * std::pow((step + 1), -epsilon_const) * H_vector[i];
