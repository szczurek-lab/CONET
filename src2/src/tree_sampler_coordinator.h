...
public:
	...
	*/
	...
	void swap_subtrees_non_descendants(NodeHandle root1, NodeHandle root2) {
		...
		nodeSet->refresh_node_data(tree->get_parent(root1));
		nodeSet->refresh_node_data(tree->get_parent(root2));
	}

	void swap_subtrees_descendants(NodeHandle parent, NodeHandle child, NodeHandle grandChild) {
	    auto old_child_parent = tree->get_parent(child);
		...
		nodeSet->refresh_node_data(tree->get_parent(child));
		nodeSet->refresh_node_data(old_child_parent);
	}
	...
	*/
	...
	Real_t getLogTreePrior() {
		...
		std::for_each(breakpoints.begin(), breakpoints.end(), [&eventsLengths, this](Event brkp) { eventsLengths += this->cells->get_event_length(brkp); });

        return -C * (Real_t)tree->get_size() - EVENTS_LENGTH_PENALTY * eventsLengths - DATA_SIZE_PRIOR_CONSTANT * ((Real_t)cells->get_cells_count()) * tree->get_size();
	}	

	void move(MoveType type) {
		if (tree_count_score == 1.0) {
			Attachment tmp_att {likelihoodCalculator->getBestAttachment()};
			tree_count_score = countsScoring.calculate_log_score(*tree, tmp_att);;
		}
		...
		Attachment after_tmp_att {likelihoodCalculator->get_after_move_best_attachment()};
		auto after_move_counts_score = countsScoring.calculate_log_score(*tree, after_tmp_att);
		if (DEBUG ) {
		...
		if (DEBUG ) {
			logDebug("Log acceptance ratio: ", logAcceptance, " ", after_move_counts_score, " ", tree_count_score, " ", getLogTreePrior(), " ", beforeMoveTreePrior);
		}
...
tree{ tree }, 
	...
	void recount_score() {
		Attachment at{likelihoodCalculator->getBestAttachment()};
		tree_count_score = countsScoring.calculate_log_score(*tree, at);
	}
...
return likelihoodCalculator->getLikelihood() +getLogTreePrior(false) + tree_count_score;
	...
	void treeMHStep() {
		...
		if (DEBUG) {
		    logDebug("Move count: ", this->moveCount);
	...
	std::tuple<EventTree, std::vector<Event>, Real_t> getBestTreeData() {
		...
		log("Getting bets tree ", TreeFormatter::to_string_representation(bestTree));
		for (auto node : bestTree.get_descendants(tree->get_root())) {
			if (at.has_attached_cells(node->label)) {
				log(node->label.first, " ", node->label.second);
			}
		}
		log("Prun start");
	    bestTree.prune_tree(bestTree.get_root(), at);
		log("Prunned best tree");
