...
private:
	 ...
	 * Where @expected_mean = m, @sum_of_values = sum x_i, @summed_squares_of_values = sum x_i^2, @number_of_values = n, @normalizer = Z
	 */
	inline Real_t calculate_l2_penalty(Real_t expected_mean, Real_t sum_of_values, Real_t summed_squares_of_values, Real_t number_of_values, Real_t normalizer) {
		return ((summed_squares_of_values - 2 * expected_mean * sum_of_values) / (normalizer) + expected_mean * expected_mean * number_of_values / normalizer);
	}
    ...
    Real_t calculate_penalty_for_bins_at_node(EventTree::NodeHandle node, std::map<Event, std::set<size_t>> &attachment) {
		...
		for (auto cell : attachment[node->label]) {
			...
			std::fill(bin_bitmap[cell].begin() + node->label.first, bin_bitmap[cell].begin() + node->label.second, true);
		}
		...
		for (const auto &cluster : cluster_to_counts_sum) {
			if (cluster_to_bin_count[cluster.first] > 0.0) {
				...
				result += COUNTS_SCORE_CONSTANT_0 * calculate_l2_penalty(mean_count, cluster.second, cluster_to_squared_counts_sum[cluster.first], bin_count, all_bins_count); 
	...
	Real_t calculate_log_score__(EventTree &tree, Attachment &at) {
		init_state();
		std::map<Event, std::set<size_t>> attachment = at.get_node_label_to_cells_map();
		Real_t result = 0.0;
