...
public:
	...
	public:
		...
		Node() : parent{nullptr} {}
		Node(TreeLabel label, std::list<Locus> nb) : parent{nullptr}, label{label}, new_breakpoints{nb} {}
		~Node()
		{
...
public:
	...
	std::list<Locus> const &getNewBreakpoints(const NodeHandle node) const { return node->new_breakpoints; }
	...
	*/
	...
	std::vector<NodeHandle> get_non_descendants(NodeHandle node) const
	{
		...
		std::set<NodeHandle> non_descendants{all_nodes.begin(), all_nodes.end()};
		for (auto d : get_descendants(node))
		{
			non_descendants.erase(d);
		}
