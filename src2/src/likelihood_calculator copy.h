...
public:
	...
	void fillLikelihoodMatrices() {
			likelihood->getLogLikelihoodBreakpoint(calculator_state.breakpoint_likelihoods, cells->get_corrected_counts());
			likelihood->getLogLikelihoodNoBreakpoint(calculator_state.no_breakpoint_likelihoods, cells->get_corrected_counts());
	}
	...
	void init_state_likelihood_matrices() {
		...
		for (size_t br = 0; br <= maxBreakpoint; br++) {
			breakpointLikelihoods[br].resize(cells->get_cells_count());
			noBreakpointLikelihoods[br].resize(cells->get_cells_count());
			buffer_breakpointLikelihoods[br].resize(cells->get_cells_count());
			buffer_noBreakpointLikelihoods[br].resize(cells->get_cells_count());
		}
	}
