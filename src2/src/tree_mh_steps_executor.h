...
using NodeHandle = EventTree::NodeHandle;
    struct MoveData {
		...
		MoveData(): label {get_root_label()}, boolean_flag {false}, move_log_kernel{1.0}, reverse_move_log_kernel{1.0} {}
	};
private:
	EventTree &tree;
    ...
    void prune_and_reattach(NodeHandle node_to_prune, NodeHandle attachment_node) {
		auto former_node_attachment = tree.prune_and_reattach(node_to_prune, attachment_node);
	...
	void swap_subtrees_descendants(NodeHandle node, NodeHandle descendant, NodeHandle descendant_of_descendant) {
	...
	void swap_breakpoints(NodeHandle node1, NodeHandle node2, int left, int right) {
		if (!label_sampler.swapOneBreakpointPossible(tree.get_node_event(node1), tree.get_node_event(node2), left, right)) {
			return;
		}
		auto new_labels = label_sampler.swapOneBreakpoint(tree.get_node_event(node1), tree.get_node_event(node2), left, right);
	...
	*/
	MoveData delete_leaf_move() {
		MoveData move_data;
		move_data.move_log_kernel = node_sampler.getDeleteLeafKernel();
		...
		move_data.reverse_move_log_kernel = node_sampler.getAddLeafKernel() + label_sampler.get_sample_label_log_kernel();
		return move_data;
	}

	MoveData add_leaf_move() { 
		MoveData move_data;
		move_data.move_log_kernel = node_sampler.getAddLeafKernel() + label_sampler.get_sample_label_log_kernel();
		move_data.nodes["added_leaf"] = add_leaf(node_sampler.sample_node(true, random), label_sampler.sample_label(random));
		move_data.reverse_move_log_kernel =  node_sampler.getDeleteLeafKernel();
		return move_data;
	}
	...
	MoveData prune_and_reattach_move() {
		...
		prune_and_reattach(node_to_prune, node_sampler.sample_non_descendant(node_to_prune, random));
		return move_data;
	}

	MoveData swap_labels_move() {
		MoveData move_data;
		auto nodes = node_sampler.sampleTwoNodes(random);
	...
	MoveData swap_subtrees_descendant_move(NodeHandle node, NodeHandle descendant) {
		MoveData move_data;
		move_data.move_log_kernel = node_sampler.getSwapSubtreesDescendantsKernel(descendant);

		auto descendant_of_descendant = node_sampler.sample_descendant(descendant, random);
		...
		move_data.reverse_move_log_kernel = node_sampler.getSwapSubtreesDescendantsKernel(node);
		return move_data;
	}

	MoveData swap_subtrees_non_descendants_move(NodeHandle node1, NodeHandle node2) {
		MoveData move_data;
	...
	void swap_breakpoints_rollback(MoveData move_data) {
		...
		if (label1.first == move_data.label.first && label1.second == move_data.label.second) {
			return;
		}
		auto label2 = tree.get_node_label(move_data.nodes["node2"]);
		if (label1.first == move_data.label.first || label1.first == move_data.label.second) {
			left = 1;
		}
		if (label2.second == move_data.label.first || label2.second == move_data.label.second) {
			right = 1;
		}
		swap_breakpoints(move_data.nodes["node1"],move_data.nodes["node2"], left, right);
	}
...
public:
	...
	void rollback_move(MoveType type, MoveData &move_data) {
		...
		case SWAP_LABELS:
			...
			return;
		case PRUNE_REATTACH:
			prune_and_reattach(move_data.nodes["prunned_root"], move_data.nodes["old_subtree_parent"]);
			return;
		case SWAP_SUBTREES:
			if (move_data.boolean_flag) {
				swap_subtrees_non_descendants(move_data.nodes["node1"], move_data.nodes["node2"]);
			} else {
				swap_subtrees_descendants(move_data.nodes["middle_node"], move_data.nodes["first_node"], move_data.nodes["parent_of_middle_node"]);
			}
			return;
