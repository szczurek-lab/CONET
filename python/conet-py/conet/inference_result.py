...
class InferenceResult:
    ...
    def get_inferred_copy_number(self, neutral_cn: int) -> np.ndarray:
        ...
        for node in nx.traversal.dfs_postorder_nodes(self.__tree, source=(0, 0)):
            ...
            summed_ccs = 0.0
            bin_count = (node[1] - node[0]) * len(attached_cells) - np.sum(bin_bitmap[range(node[0], node[1]), :][:, attached_cells]) # non zero elements in bit map
    ...
    def __get_pretty_tree(self) -> nx.DiGraph:
        def __node_to_pretty(node: Tuple[int, int]) -> Dict:
            if node == (0,0):
                return {}
            return {
        ...
        pretty_tree.add_edges_from(
            [(json.dumps(__node_to_pretty(e[0])), json.dumps(__node_to_pretty(e[1])))
             for e in self.__tree.edges]
        )
        return pretty_tree
    ...
    def __load_inferred_tree(self, path: str) -> nx.DiGraph:
        ...
        with open(path) as f:
            edges = [(__int_tuple_from_str(line.split('-')[0]), __int_tuple_from_str(line.split('-')[1])) for line in f.readlines()]
            edges = [((brkp_candidates[e[0][0]], brkp_candidates[e[0][1]]), (brkp_candidates[e[1][0]], brkp_candidates[e[1][1]])) for e in edges]
            tree = nx.DiGraph()
    ...
    def __load_attachment(self, path: str) -> List[Tuple[int, int]]:
        brkp_candidates = self.__cc.get_brkp_candidate_loci_idx()
        with open(path) as f:
            return [(brkp_candidates[int(line.split(';')[2])], brkp_candidates[int(line.split(';')[3])]) for line in f.readlines()]
